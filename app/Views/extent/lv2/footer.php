...
<?php    
    }elseif ($menu == "1x") { //data transaksi booking
?>
        ...
        <script>
                    ...
                    buttons: [                      
                        {
                            ...
                            className: 'btn btn-danger',
                            action: function ( e, dt, node, config ) {
                                $("#_actpelanggan").attr('action', '<?=base_url()?>/pegawai/add');
                                $('#add_pelanggan').modal('show');   
                                $(".title-add-pelanggan").html('Tambah Pegawai');
                                $("#firstname").val("");  
                                $("#lastname").val("");  
                                $("#tim").val("");  
                                $("#phone1").val(""); 
            ...
            /*     $("#provinsi").on("select2:select", function (e) { 
                ...
                $(document).on("click", ".btn--pelanggan", function () {  
                    ...
                    $.getJSON("<?=base_url()?>/pegawai/e", data, function (result) { 
                      

                        $.each(result.tampil, function() {
                            $(".title-add-pelanggan").html("Edit Pegawai"); 
                            $("#_actpelanggan").attr('action', '<?=base_url()?>/pegawai/edit');
                            if (this['gambar'] != "") {
                                $("#gambarnya").attr('src', '<?=base_url()?>/uploads/' + this['gambar'] );
                            } 
                            $("#firstname").val( this['firstname'] );  
                ...
                function kecamatan_id(kabupaten = null, kecamatan = null ) { 
                    var url = "<?php echo site_url('pegawai/add_ajax_kc');?>/" + kabupaten + "*" + kecamatan ;
                    $('#kecamatan').load(url);
                    return false;
                }
